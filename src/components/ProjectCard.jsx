...
export default function ProjectCard({ project, index }) {
  ...
  return (
    <motion.div
      ...
      className={`glass-card relative overflow-hidden p-6 cursor-none ${
    ...
    >
      ...
      <div
        ...
        style={{
          ...
          background: `radial-gradient(circle at ${glowPos.x}% ${glowPos.y}%, rgba(230,57,70,0.6), transparent 60%)`,
        }}
      />

      {/* Featured Badge */}
      {project.featured && (
        <span className="absolute top-4 right-4 bg-primary/20 text-primary text-xs font-heading font-semibold px-3 py-1 rounded-full border border-primary/40">
          Featured ⭐
        </span>
      )}
      ...
      <div className="relative z-10">
        <h3 className="font-heading text-xl font-bold text-text-primary mb-3 pr-24">
          {project.title}
        </h3>
        ...
        <div className="flex flex-wrap gap-2 mb-4">
          {project.tags.map((tag, i) => (
            <span
              key={i}
              className="text-xs font-body px-2 py-1 rounded bg-primary/10 text-primary border border-primary/20"
            >
              {tag}
            </span>
          ))}
        ...
        >
          ...
          <a
            ...
            className="inline-flex items-center gap-2 text-sm font-heading font-medium text-primary hover:text-secondary transition-colors cursor-none"
          >
          ...
          {/* Live Demo Button — only shown when liveUrl exists */}
          {project.liveUrl && (
            <a
